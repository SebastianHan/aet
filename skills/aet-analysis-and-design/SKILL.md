...
description: |
  An end-to-end software requirements design workflow that produces a complete design solution through a three-phase process: "Socratic Dialogue + architectural design + SDD task breakdown". Use when: (1) requirements are unclear or need decomposition, (2) implementation involves architecture, system, or DFx detailed design, (3) visualized system design documents (change scope and boundaries) are needed, or any other requirements development, system design, and development plan generation tasks.
metadata:
  pattern: pipeline
  stages: 3
...
**Load Reference**:
- **Load SKILL**: Load `aet-analysis-and-design` skill - NEVER execute work without loading the skill first
- **Reference**: Read `aet-analysis-and-design/references/reviewer.md` for review 
