...
<!-- instruction: NEVER make design decisions. Breaking down functional requirements to the implementation level, which restricts subsequent design flexibility and makes it difficult to adapt to user-driven changes. -->
...
<!-- instruction: Record the user's original description faithfully, DO NOT change, optimize, or delete, so that it can be traced and referenced during subsequent design and development processes. -->

```text
[User's original requirement description, can be used for traceability and reference during design and development]
```

### 7.2 用户澄清纪录

<!-- instruction: Faithfully record user answers and document revisions without altering, optimizing, or deleting them for future design and development traceability. This includes interview Q&A and review-stage modifications/annotations. -->

```text
[Questions asked and answers received during user interviews]
```

```text
[User modifications and annotations made during the review stage]
```
